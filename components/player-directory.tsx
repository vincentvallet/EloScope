...
export function PlayerSearchPage() {
  ...
  return <div className="plain-page players-page">
    ...
    <div className="player-search-results">{payload?.items.map((player) => <a href={`/joueurs/${player.ffeCode}`} className="player-search-result" key={player.ffeCode}>
      <Avatar name={player.displayName}/><div><h2>{player.federationFlag && <span role="img" aria-label={`Drapeau de la fédération ${player.federationName ?? player.federationCode}`}>{player.federationFlag} </span>}{player.fideTitle && <span className="status-pill" title={player.fideTitleLabel}>{player.fideTitle}</span>} {player.displayName}</h2><p><strong>{player.ffeCode}</strong>{player.currentClubName ? ` · ${player.currentClubName}` : ""}</p><small>{player.category ?? "Catégorie non publiée"} · Elo {player.standardRating ?? "NC"} · {player.federationName ?? player.federation ?? "Fédération non publiée"}{player.birthYear ? ` · Né(e) en ${player.birthYear}` : ""} · {player.indexedTournamentCount} tournoi(s) indexé(s)</small></div><ArrowRight/>
    </a>)}</div>
...
export function PlayerProfilePage({ ffeCode }: { ffeCode: string }) {
  ...
  return <div className="plain-page players-page">
    ...
    <div className="player-profile-head"><Avatar name={profile.displayName}/><div><span className="status-pill">{profile.ffeCode}</span>{profile.fideId && <span className="status-pill">FIDE {profile.fideId}</span>}<h1>{profile.federationFlag && <span role="img" aria-label={`Drapeau de la fédération ${profile.federationName ?? profile.federationCode}`}>{profile.federationFlag} </span>}{profile.displayName}</h1><p>{profile.fideTitle && <><strong title={profile.fideTitleLabel}>{profile.fideTitle}</strong> · {profile.fideTitleLabel} · </>}{profile.currentClubName ?? "Club non publié"} · Fédération représentée : {profile.federationName ?? profile.federation ?? "non publiée"}{profile.birthYear ? ` · Né(e) en ${profile.birthYear}` : ""}</p></div></div>
